class UnfairProphetAlgorithm(object):
    """docstring for UnfairProphetAlgorithm."""

    def __init__(self):
        pass

    def ComputeSolutionOneHalf(self, elements, distributions, q):
    ...
    def ComputeSolutionMinusOneE(self, elements, distributions, q):
        for el in elements:
    ...
    def ComputeSolutionThreeForth(self, elements, distributions, q):
        for el in elements:
            cond = distributions[el.type].PThreshold(el)
            if el.value >= cond:
                return el
    ...
    def ComputeSolutionDiffEq(self, elements, distributions, q):
        diff_solution_50 = [
        ...
        for i in range(elements):
            pow = diff_solution[i] ** (1 / (len(elements) - 1)
            cond = distributions[elements[i].type].Reverse(pow))
